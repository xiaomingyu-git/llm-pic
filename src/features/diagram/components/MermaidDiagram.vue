...
const generateDiagram = async () => {
  ...
  try {
    ...
    if (!mermaidInitialized) {
      ElMessage.error('Mermaid库未初始化，请刷新页面重试');
      return;
    }
...
const renderDiagram = async () => {
  ...
  try {
    ...
    // 使用简化的渲染方式，避免解构赋值可能的问题
    const renderResult = await mermaid.render(graphId, diagramCode.value);

    // 设置SVG内容，兼容不同的返回格式
    if (typeof renderResult === 'string') {
      diagramContainer.value.innerHTML = renderResult;
    } else if (renderResult.svg) {
      diagramContainer.value.innerHTML = renderResult.svg;
    } else {
      diagramContainer.value.innerHTML = renderResult;
    }
