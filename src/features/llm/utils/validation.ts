...
import type { DiagramGenerationOptions } from '../../diagram/types';
...
export class ValidationService {
  ...
  static isValidApiKey(
    ...
    // 自动检测
    return (
      API_KEY_PATTERNS.OPENAI.test(key) ||
      API_KEY_PATTERNS.CLAUDE.test(key) ||
      API_KEY_PATTERNS.CUSTOM.test(key)
    );
  }

  static getApiKeyProvider(
    key: string
  ): 'openai' | 'claude' | 'custom' | 'unknown' {
  ...
  static validateDiagramOptions(options: Partial<DiagramGenerationOptions>): {
    ...
    if (
      options.theme &&
      !['default', 'dark', 'forest', 'neutral'].includes(options.theme)
    ) {
      errors.push('图表主题无效');
    }
    ...
    if (
      options.direction &&
      !['TB', 'TD', 'BT', 'RL', 'LR'].includes(options.direction)
    ) {
      errors.push('图表方向无效');
    }
  ...
  static validateDiagramInput(input: string): {
    ...
    // 提供改进建议
    if (
      !trimmedInput.includes('系统') &&
      !trimmedInput.includes('应用') &&
      !trimmedInput.includes('架构')
    ) {
      suggestions.push('建议包含"系统"、"应用"或"架构"等关键词');
    }

    if (
      !trimmedInput.includes('数据库') &&
      !trimmedInput.includes('API') &&
      !trimmedInput.includes('服务')
    ) {
