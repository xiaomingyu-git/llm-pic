...
export class ApiService {
  ...
  async generateDiagram(
    prompt: string,
    options: Partial<LLMRequestParams> = {}
  ): Promise<LLMResponse> {
    const requestBody: LLMRequestParams = {
      model: options.model || 'gpt-3.5-turbo',
      messages: [
        {
          role: 'system',
          content:
            '你是一个架构图生成专家。请根据用户的描述生成标准的Mermaid格式架构图代码。只返回代码，不要包含其他说明。',
        },
        {
          role: 'user',
  ...
  getConfig(): LLMConfiguration {
    return {
      ...
      apiKey:
        (
          this.axiosInstance.defaults.headers.common['Authorization'] as string
        )?.replace('Bearer ', '') || '',
    };
  }
}
...
export class ApiError extends Error implements ApiErrorInfo {
  ...
  constructor(message: string, code: string, originalError?: any) {
    ...
    this.timestamp = originalError?.timestamp || new Date();
    this.requestUrl = originalError?.requestUrl;
    this.requestMethod = originalError?.requestMethod;
    this.responseTime = originalError?.responseTime;
  }
}
