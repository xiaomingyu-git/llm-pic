export class CopyService {
    ...
    try {
      ...
      if (success) {
        onSuccess?.(
          text.length > 100
            ? '内容已复制到剪贴板'
            : `已复制: ${text.substring(0, 50)}${text.length > 50 ? '...' : ''}`
        );
      } else {
   ...
   * 复制图片到剪贴板
   */
  static async copyImageToClipboard(
   ...
   * 将图片转换为 Canvas
   */
  private static async imageToCanvas(
...
export const copyImageToClipboard =
  CopyService.copyImageToClipboard.bind(CopyService);
export const copySVGToClipboard =
  CopyService.copySVGToClipboard.bind(CopyService);
