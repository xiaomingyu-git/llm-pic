import type { XMLNode, XMLEdge, XMLDiagramData } from '../types'
          ...
          const mockDocument: MockDocument = {
            querySelector: (selector: string) => {
              if (selector === 'parsererror') {
                return xmlString.includes('>') ? null : { nodeType: 1 } as any
              }
              return null
            },
            querySelectorAll: (_selector: string) => {
              ...
              lines.forEach((line, index) => {
                if (line.includes('<node') || line.includes('<component') || line.includes('<service') || line.includes('<database')) {
                  const labelMatch = line.match(/>([^<]*?)</) || ['', line.match(/label="([^"]*?)"/)?.[1] || `节点${index + 1}`]
...
export class XMLDiagramService {
  ...
  static parseXML(xmlString: string): XMLDiagramData {
    try {
      ...
      // 解析节点
      const nodes: XMLNode[] = []
      const nodeElements = xmlDoc.querySelectorAll('node, component, service, database')

      nodeElements.forEach((nodeElement: MockElement, index: number) => {
        const node: XMLNode = {
          id: nodeElement.getAttribute('id') || `node_${index}`,
          label: nodeElement.getAttribute('label') || nodeElement.textContent || `节点 ${index + 1}`,
          x: parseInt(nodeElement.getAttribute('x') || '0', 10),
          y: parseInt(nodeElement.getAttribute('y') || (index * 80).toString(), 10),
      ...
      return {
        ...
        edges,
        metadata: {
          width: parseInt(xmlDoc.querySelector('diagram')?.getAttribute('width') || '800', 10),
          height: parseInt(xmlDoc.querySelector('diagram')?.getAttribute('height') || '600', 10)
        }
      }
    } catch (error) {
  ...
  static renderToHTML(diagram: XMLDiagramData): string {
    ...
    // 计算图表边界
    const maxX = Math.max(...diagram.nodes.map(node => (node.x || 0) + (node.width || 120)))
    const maxY = Math.max(...diagram.nodes.map(node => (node.y || 0) + (node.height || 60)))
  ...
  private static getNodeShapeClass(node: XMLNode): string {
    switch (node.type) {
      case 'database': return 'xml-node-database'
      case 'server': return 'xml-node-server'
      case 'client': return 'xml-node-client'
      case 'circle': return 'xml-node-circle'
      default: return 'xml-node-rect'
    }
  }
   ...
   * 创建连接线SVG
   */
  private static createConnection(fromNode: XMLNode, toNode: XMLNode, edge: XMLEdge): string {
    ...
    // 创建连接线路径
    const pathClass = edge.type === 'dashed' ? 'xml-edge-dashed' : 'xml-edge-line'
