            ...
            <el-card>
              ...
              <el-form :model="mermaidInput" label-width="80px">
                <el-form-item label="描述">
                  <el-input
                    v-model="mermaidInput.description"
                    type="textarea"
                    :rows="4"
          ...
          <div class="diagram-section">
            <el-card>
              ...
              <el-form :model="xmlInput" label-width="80px">
                <el-form-item label="描述">
                  <el-input
                    v-model="xmlInput.description"
                    type="textarea"
                    :rows="4"
                    placeholder="描述您想要生成的系统架构，XML格式支持精确的布局控制"
                    maxlength="500"
                    show-word-limit
                  />
                </el-form-item>
...
import { CopyDocument, QuestionFilled, Setting, Refresh, InfoFilled, FullScreen } from '@element-plus/icons-vue'
...
请生成对应的XML架构图代码：`
    ...
    if (result.choices && result.choices.length > 0 && result.choices[0].message) {
      ...
      if (result.choices[0].message.reasoning_content) {
        console.log('XML模型思考过程:', result.choices[0].message.reasoning_content)
      }
      ...
      if (finalCode.includes('```xml')) {
        finalCode = finalCode.replace(/```xml\s*/g, '').replace(/```\s*$/g, '').trim()
      } else if (finalCode.includes('```')) {
        finalCode = finalCode.replace(/```\s*/g, '').replace(/```\s*$/g, '').trim()
      }
      ...
      ElMessage.success(`XML架构图生成成功 (模型: ${result.model || 'unknown'}, 耗时: ${result.usage?.total_tokens || 0} tokens)`)
    } else {
      throw new Error('LLM API返回格式错误')
    }
  } catch (error) {
