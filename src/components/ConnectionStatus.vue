  ...
  <div class="connection-status">
    ...
    <div v-if="lastConnectionTime" class="last-connection">
      <el-tooltip :content="formatFullTime(lastConnectionTime)" placement="top">
        <span class="time-text">最后连接: {{ formatRelativeTime(lastConnectionTime) }}</span>
      </el-tooltip>
    </div>
  </div>
</template>
...
import { CircleCheck, Loading, Connection, CircleClose, Warning } from '@element-plus/icons-vue'
...
// Emits
interface Emits {
  (e: 'status-change', status: 'disconnected' | 'testing' | 'connected' | 'error'): void
...
// 公共方法
const testApiConnection = async (config?: LLMConfiguration): Promise<boolean> => {
...
// 获取模型列表
const fetchAvailableModels = async (config?: LLMConfiguration): Promise<string[]> => {
  const testConfig = config || props.config
...
// 验证API端点
const validateApiEndpoint = async (config: LLMConfiguration): Promise<boolean> => {
  try {
    // 直接使用配置的URL，不验证格式
...
// 获取API信息
const getApiInfo = async (config: LLMConfiguration): Promise<ConnectionDetails | null> => {
  try {
...
// 私有方法
const setStatus = (status: 'disconnected' | 'testing' | 'connected' | 'error') => {
