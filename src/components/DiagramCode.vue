...
const highlightedCode = computed(() => {
  return highlightCode(editableCode.value, detectedLanguage.value, selectedTheme.value)
})
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'Tab') {
    ...
    editableCode.value = value.substring(0, start) + '  ' + value.substring(end)
...
const highlightCode = (code: string, language: string, theme: string): string => {
  if (!code) return ''
  ...
  const keywords = {
    mermaid: [
      'graph', 'flowchart', 'subgraph', 'end', 'linkStyle', 'classDef', 'class',
      'click', 'callback', 'hypertext', 'loadjavascript', 'init', 'config'
    ],
    plantuml: [
      '@startuml', '@enduml', 'component', 'interface', 'package', 'node',
      'database', 'actor', 'usecase', 'class', 'object', 'sequence', 'state'
    ]
  }

  const langKeywords = keywords[language as keyof typeof keywords] || keywords.mermaid
  ...
  // 高亮箭头和连接符
  highlighted = highlighted.replace(/([-->|<--|<-->|\.\.|\.\.\.])/g, '<span class="arrow">$1</span>')
